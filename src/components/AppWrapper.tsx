...
export default function AppWrapper({ children }: {
    ...
    return (
        <div className="flex h-screen w-full overflow-hidden bg-transparent">
            ...
            <nav
                className={`${isSidebarOpen ? "w-full md:w-72 translate-x-0" : "w-full md:w-0 -translate-x-full"
            ...
            {/* Main Content Area (Acá se renderizan los children) */}
            <main className="flex-1 flex flex-col relative w-full h-full min-w-0 bg-transparent">
                ...
                {/* ESTO ES MAGIA DE NEXT.JS: Aquí inyecta el contenido de la ruta actual (ej: page.tsx) */}
                {children}
            </main>
        </div>
    )
